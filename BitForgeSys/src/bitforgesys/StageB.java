...
/**
 * This class represents the Stage B of the project and is used to perform
...
public class StageB {
    ...
    /**
     * This is main gateway to execute the Stage B
     *
    ...
    public static void main(String[] args) {
        ...
        // creating instance of Stage B
        StageB demo = new StageB();
    ...
    private void menu() {
        ...
        while (true) {
            ...
            System.out.println(" ***********************************");
            System.out.println("1- Add a processed print job to the back of the queue");
            System.out.println("2- List print ID, customer ID, description of jobs");
            System.out.println("3- Search for Processed Print Job");
    ...
    private void addJob() {
        ...
        System.out.print("Enter plastic type i.e. PLA (default), ABS, Nylon or Acrylic:");
        String plasticType = inputSource.nextLine();

        // If user have not chosen the ABS, NYLON or acrylic as plastic type, user PLA as plastic type by default
        if (!plasticType.equalsIgnoreCase("abs") && !plasticType.equalsIgnoreCase("nylon")
                && !plasticType.equalsIgnoreCase("acrylic")) {
            plasticType = "PLA";
        }

        // variables for storing user's input regarding UV curing, UV coating and polishing
        boolean uvCuring, uvCoating, polishing;

        // Ask user if s/he wish to apply UV curing
        System.out.print("Do you wish to apply UV Curing, Y or N? ");
        String uvCuringInput = inputSource.nextLine();

        // If yes then set respective flag to true
        if (uvCuringInput.equalsIgnoreCase("y")) {
            uvCuring = true;
        ...
        // Ask user if s/he wish to apply UV coating
        System.out.print("Do you wish to apply UV Coating, Y or N? ");
        String uvCoatingInput = inputSource.nextLine();
        // If yes then set respective flag to true
        if (uvCoatingInput.equalsIgnoreCase("y")) {
            uvCoating = true;
        } // set flag to false otherwise
        else {
            uvCoating = false;
        ...
        // Ask user if s/he wish to apply polishing
        System.out.print("Do you wish to apply Polishing, Y or N? ");
        String polishingInput = inputSource.nextLine();
        // If user has chosen not to apply UV coating and curing then 
        // apply polishing anyway as one processing is mandatory
        if (polishingInput.equalsIgnoreCase("y") || uvCoating == false && uvCuring == false) {
            polishing = true;
        } else {
            polishing = false;
        }
        ...
        PrintJob pj = new ProcessedPrintJob(uvCuring, uvCoating, polishing, this.printQueue.currentPrintJobID,
